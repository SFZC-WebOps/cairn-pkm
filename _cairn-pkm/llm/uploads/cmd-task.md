...
**Optional:** due_date, assignee, phase, effort, viz, type[], parent_task
...
| viz | "active work/discuss" → now; "keep eyes on/monitor" → next; "soon/near-term" → soon; "backlog/later" → later; "stuck/can't proceed" → blocked; "waiting on" → waiting |

---

## Understanding viz vs status

**viz** (visibility) = How tasks surface in dashboards
- Controls **attention and workflow**
- Values: `now`, `next`, `soon`, `later`, `blocked`, `waiting`
- Question: "When should I look at this?"

**status** (state) = Current task state
- Controls **task lifecycle**
- Values: `active`, `blocked`, `complete`, `deferred`, `onhold`, `scheduled`, `waiting`
- Question: "What state is this in?"

**Example combinations:**
- `viz: now` + `status: active` = Working on this right now
- `viz: next` + `status: active` = Monitor this, will work on it soon
- `viz: blocked` + `status: blocked` = Can't proceed, needs attention
- `viz: later` + `status: scheduled` = Backlog item, scheduled for future

**Default behavior:** Tasks without `viz` won't appear in focused dashboard views.
...
|---------|------|---------|
| 0.10.0 | 2025-12-20 | Added viz field with plain text values, clarified viz vs status |
