...
### 4. Configure LLM Commands

Cairn-PKM includes LLM commands for working with your vault conversationally.

**To configure:**

1. Edit `_cairn-pkm/llm/uploads/cairn-pkm-user-prefs.yaml`:
...
2. Upload files from `_cairn-pkm/llm/uploads/` to your LLM project:
   - `cairn-pkm-user-prefs.yaml` (your settings)
   - All `cmd-*.md` files (command specs)
   - `_ARCHITECTURE.md` and `_INSTALLATION.md` (reference docs)

**Choose your output mode:**

| Your Setup | Setting |
|------------|---------|
| Web-based LLM OR local filesystem | `file_operations: "download"` |
| Desktop app with MCP/filesystem access | `file_operations: "write"` |

If unsure, use `download` — you can change it later.
...
- `!tour` - 5-minute guided walkthrough

See `_cairn-pkm/llm/commands/` for complete command documentation.

**Note:** You can use Cairn-PKM without LLM commands - the vault structure, templates, and Dataview queries work independently.

### 5. Explore Example Content


- **Tracks/area-admin/** â€” Example area
- **Tracks/p002-prod-lnch/** â€” Example project
...
- `!tour` â€” 5-minute guided walkthrough (recommended for new users)
- `!hi` â€” Open interactive session menu
